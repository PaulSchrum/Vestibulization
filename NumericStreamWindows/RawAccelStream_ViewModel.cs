...
namespace Vestibulization
{
   ...
   {
      RxSpatial.SpatialDataStreamer_raw2 accelStream_raw2=null;
      ...
      private void ctor_Raw2Stream()
      {
         accelStream_raw2 = SpatialDataStreamer_raw2.Create();
         StreamSubscription = 
            accelStream_raw2
            .DeviceDataStream.Subscribe(this);
         accelStream_raw2.AttachedStateChanged += AttachedStateChanged;
         accelStream_raw2.CalibratingStateChanged += CalibratingStateChanged;
         OptionValue = "Raw 2";
      }

      private void ctor_RawStream()
      {
         accelStream_raw = new SpatialDataStreamer_raw();
         StreamSubscription = accelStream_raw.Subscribe(this);
      ...
      private void WriteStateChanged(Object sender, EventArgs e)
      {
         if (null == e) return;
         WriteState writeState = (e as WriteStateEventArg).WriteState;
         switch (writeState)
         {
            case WriteState.Closed:
            {
               OptionText = "Clsd Write State";
               break;
            }
            case WriteState.NotOpenYet:
            {
               OptionText = "NY Open Write State";
               break;
            }
            case WriteState.OpenNow:
            {
               OptionText = "Open Now Write State";
               break;
            }
         }
      }
